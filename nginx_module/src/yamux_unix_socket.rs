//! A standalone duplicate of [`crate::unix_socket`] for the yamux daemon proxy.
//!
//! This intentionally duplicates the `UnixSocket` implementation so the yamux
//! transport can evolve independently of the one used by the v2a/v2b daemon
//! proxies. It is currently a faithful copy exposing the type as
//! [`YamuxUnixSocket`]: it performs a connection-level handshake on connect, then
//! hands every subsequent inbound byte to `read_event` (which feeds the yamux
//! session). `after_handshake` fires on each successful (re)connect so the caller
//! can (re)initialise per-connection state — e.g. rebuild the yamux session.
...
pub struct YamuxUnixSocket(Pin<Box<Inner>>);
